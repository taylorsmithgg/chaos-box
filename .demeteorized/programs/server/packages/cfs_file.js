...
Meteor.methods({                                                                                                      // 176
  ...
  '_cfs_getUrlInfo': function (url, options) {                                                                        // 180
    this.unblock();                                                                                                   // 181
                                                                                                                      // 182
    var response = HTTP.call("HEAD", url, options);                                                                   // 183
    var headers = response.headers;                                                                                   // 184
    var result = {};                                                                                                  // 185
                                                                                                                      // 186
    if (headers['content-type']) {                                                                                    // 187
      result.type = headers['content-type'];                                                                          // 188
    }                                                                                                                 // 189
                                                                                                                      // 190
    if (headers['content-length']) {                                                                                  // 191
      result.size = +headers['content-length'];                                                                       // 192
    }                                                                                                                 // 193
                                                                                                                      // 194
    if (headers['last-modified']) {                                                                                   // 195
      result.updatedAt = new Date(headers['last-modified']);                                                          // 196
    }                                                                                                                 // 197
                                                                                                                      // 198
    return result;                                                                                                    // 199
  }                                                                                                                   // 200
                                                                                                                      ...
                                                                                                                      // 202
// TODO maybe this should be in cfs-storage-adapter                                                                   // 203
function _copyStoreData(fileObj, storeName, sourceKey, callback) {                                                    // 204
  if (!fileObj.isMounted()) {                                                                                         // 205
    throw new Error("Cannot copy store data for a file that is not associated with a collection");                    // 206
  }                                                                                                                   // 207
                                                                                                                      // 208
  var storage = fileObj.collection.storesLookup[storeName];                                                           // 209
  if (!storage) {                                                                                                     // 210
    throw new Error(storeName + " is not a valid store name");                                                        // 211
  }                                                                                                                   // 212
                                                                                                                      // 213
  // We want to prevent beforeWrite and transformWrite from running, so                                               // 214
  // we interact directly with the store.                                                                             // 215
  var destinationKey = storage.adapter.fileKey(fileObj);                                                              // 216
  var readStream = storage.adapter.createReadStreamForFileKey(sourceKey);                                             // 217
  var writeStream = storage.adapter.createWriteStreamForFileKey(destinationKey);                                      // 218
                                                                                                                      // 219
  writeStream.once('stored', function(result) {                                                                       // 220
    callback(null, result.fileKey);                                                                                   // 221
  });                                                                                                                 // 222
                                                                                                                      // 223
  writeStream.once('error', function(error) {                                                                         // 224
    callback(error);                                                                                                  // 225
  });                                                                                                                 // 226
                                                                                                                      // 227
  readStream.pipe(writeStream);                                                                                       // 228
}                                                                                                                     // 229
var copyStoreData = Meteor.wrapAsync(_copyStoreData);                                                                 // 230
                                                                                                                      // 231
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
